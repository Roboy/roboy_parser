...
public final class TypeInference {
  ...
  private static class Env {
    ...
    public SemType updateType(String var, SemType type) {
      ...
      if (!newType.isValid() && opts.verbose >= 2)
        NLULoggerController.warnings("Invalid type from [%s MEET %s]", ref.value, type);
      ref.value = newType;
      return newType;
    }
  ...
  public static SemType inferType(Formula formula, TypeLookup typeLookup, boolean allowFreeVariable) {
    ...
    if (opts.verbose >= 2) NLULoggerController.logs("TypeInference: %s => %s", formula, type);
    return type;
  }
  ...
  private static SemType inferType(Formula formula, Env env, SemType type) throws TypeException {
    if (opts.verbose >= 5)
      NLULoggerController.logs("TypeInference.inferType(%s, %s, %s)", formula, env, type);
