...
class SparqlBlock implements SparqlExpr {
  ...
  private boolean isPrimitiveType(String id) {
    for (String key : CanonicalNames.PRIMITIVES) {
      if (key.equals(id)) {
        return true;
      }
    }
  ...
  void addStatement(PrimitiveFormula arg1, String property, PrimitiveFormula arg2, boolean optional) {
    ...
    try{
      ...
      NLULoggerController.errors("Exception: %s", e);
    }
    ...
    if (property.equals(DatabaseInfo.getType(property,"TYPE"))) {
      String id = getId(arg2);
      if (isPrimitiveType(id) || DatabaseInfo.getType(property,"ANY").equals(id)) return;
    }
