...
public class RemoteStringCache implements StringCache {
  ...
  public RemoteStringCache(String path, String host, int port) {
    try {
      LogController.begin_track("RemoteStringCache: connecting to %s:%s to access %s", host, port, path);
      this.socket = new Socket(host, port);
      this.out = new PrintWriter(socket.getOutputStream(), true);
      this.in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
      String response = makeRequest("open", path, null);
      LogController.logs("Using cache path=%s, host=%s, port=%s", path, host, port);
    ...
    try {
      ...
      for (int i = 0; i < NUM_TRIES; i++) {
        try {
          ...
          LogController.logs("RemoteStringCache.makeRequest(%s, %s, %s) failed", method, key, value);
        }
      }
