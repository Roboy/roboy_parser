 ...
 * @maintainer emlozin
 */
public final class DbFormulasInfo {
  // Everyone should use the singleton.
  private static DbFormulasInfo dbFormulaInfo;

  public static DbFormulasInfo getSingleton() {
    if (dbFormulaInfo == null) {
      dbFormulaInfo = new DbFormulasInfo();
    }
    return dbFormulaInfo;
  }
  ...
  private DbFormulasInfo() {
    try {
      databaseInfo = DatabaseInfo.getSingleton();
      loadFormulaInfo();
  ...
  /**
   * Map that given a type provides all Database predicates that have that type
  ...
  private void loadFormulaInfo() throws IOException {
    ...
    binaryFormulaInfoMap = databaseInfo.createBinaryFormulaInfoMap();
    unaryFormulaInfoMap = databaseInfo.createUnaryFormulaInfoMap();
  ...
  private boolean isLegalCvt(Formula inner, Formula outer) {
    if (DatabaseInfo.isReverseProperty(inner.toString()) && !DatabaseInfo.isReverseProperty(outer.toString()))
      return false;
    if (!DatabaseInfo.isReverseProperty(inner.toString()) && DatabaseInfo.isReverseProperty(outer.toString()))
      return false;
    return true;
  ...
  private boolean hasOpposite(LispTree tree) {
    if (tree.isLeaf()) {
      String dbProperty = DatabaseInfo.isReverseProperty(tree.value) ? tree.value.substring(1) : tree.value;
      return databaseInfo.propertyHasOpposite(dbProperty);
    } else {
      // Un-reverse everything.
      String binary1 = tree.child(2).child(0).value;
      binary1 = DatabaseInfo.isReverseProperty(binary1) ? binary1.substring(1) : binary1;
      String binary2 = tree.child(2).child(1).child(0).value;
      binary2 = DatabaseInfo.isReverseProperty(binary2) ? binary2.substring(1) : binary2;
      return databaseInfo.propertyHasOpposite(binary1) && databaseInfo.propertyHasOpposite(binary2);
    }
  }
  ...
  public boolean isReversed(Formula formula) {
    ...
    if (tree.isLeaf())
      return DatabaseInfo.isReverseProperty(tree.value);
    else
      return DatabaseInfo.isReverseProperty(tree.child(2).child(0).value);
  }
  ...
  public Formula equivalentFormula(LispTree tree) {
    ...
    if (tree.isLeaf()) {
      boolean rev = DatabaseInfo.isReverseProperty(tree.value);
      String dbProperty = rev ? tree.value.substring(1) : tree.value;
      String oppositeProperty = databaseInfo.getOppositeFbProperty(dbProperty);
      ...
      String binary2 = tree.child(2).child(1).child(0).value;
      binary2 = DatabaseInfo.isReverseProperty(binary2) ? binary2.substring(1) : binary2;
      String oppositeBinary1 = databaseInfo.getOppositeFbProperty(binary1);
      String oppositeBinary2 = databaseInfo.getOppositeFbProperty(binary2);
      boolean rev = DatabaseInfo.isReverseProperty(tree.child(2).child(0).value);
  ...
  public boolean isCvt(String type) {
    return databaseInfo.isCvt(type);
  }
  ...
  public class FormulaByFeaturesComparator implements Comparator<Formula> {
    ...
    public int compare(Formula f1, Formula f2) {
      ...
      FeatureVector features1 = BridgeFn.getBinaryBridgeFeatures(dbFormulaInfo.getBinaryInfo(f1));
      FeatureVector features2 = BridgeFn.getBinaryBridgeFeatures(dbFormulaInfo.getBinaryInfo(f2));
  ...
  public static class BinaryFormulaInfo {
    ...
    public static List<String> tokenizeFbDescription(String dbDesc) {
      List<String> res = new ArrayList<>();
      String[] tokens = dbDesc.split("\\s+");
