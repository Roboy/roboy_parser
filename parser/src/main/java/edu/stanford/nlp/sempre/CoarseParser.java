...
public class CoarseParser {
  ...
  public CoarseParser(Grammar grammar) {
    ...
    LogController.logs("Coarse parser: %d catUnaryRules (sorted), %d nonCatUnaryRules", catUnaryRules.size(), grammar.rules.size() - catUnaryRules.size());
  }
  ...
  class CoarseParserState {
    ...
    private void addToChart(String cat, int start, int end) {
      if (Parser.opts.verbose >= 5)
        LogController.logs("Adding to chart %s(%s,%s)", cat, start, end);
    ...
    private void addToChart(String parentCat, String childCat, int start, int end) {
      if (Parser.opts.verbose >= 5)
        LogController.logs("Adding to chart %s(%s,%s)-->%s(%s,%s)", parentCat, start, end, childCat, start, end);
    ...
    private void addToChart(String parentCat, String leftCat, String rightCat, int start, int i, int end) {
      if (Parser.opts.verbose >= 5)
        LogController.logs("Adding to chart %s(%s,%s)-->%s(%s,%s) %s(%s,%s)", parentCat, start, end, leftCat, start, i, rightCat, i, end);
