...
public final class DatabaseInfo {
  ...
  public static class Options {
    ...
    public static void setDefault(String formula){
      //LogController.logs("Default %s", formula);
      defaultDB = formula;};
  ...
  public String typeToUnit(String type, String property) {
    if (type == null) {
      // LogController.errors("No type information for property: %s", property);
      return null;
    }
    if (type.equals(getType(property,"ID")) || type.equals(getType(property,"FLOAT"))) {
      String unit = unit2Map.get(property);
      if (unit == null) {
        // LogController.errors("No unit information for property: %s", property);
...
//      }
    ...
    // LogController.logs("Warning: invalid Database uri!: %s", uri);
...
//        LogController.logs("%s", mapTypes.toString());
      }
      catch (Exception e) {
        throw new RuntimeException(e);
      }
    }
//    LogController.logs("Hello there: %s -- %s", head, id);
    if (head.contains(mapTypes.get("fb").get("PREFIX"))||
        opts.getDefault().contains(mapTypes.get("fb").get("PREFIX"))){
//      LogController.logs("Freebase: %s", id);
...
//      LogController.logs("DBpedia: %s -> %s", id, mapTypes.get("dbpedia").get(id));
