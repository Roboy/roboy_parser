            ...
            try {
              ...
              if (opts.allowedRange != null) {
                if (numberValue.value < opts.allowedRange.get(0) || numberValue.value > opts.allowedRange.get(1)) {
                  NLULoggerController.warnings("NumberFn: %f is outside of the allowed range %s", numberValue.value, opts.allowedRange);
                  return null;
                }
              }
              ...
              NLULoggerController.warnings("NumberFn: Cannot convert NerSpan \"%s\" to a number", value);
            }
          }
        }
